...
from app import app, parse_metar, build_summary, decode_weather_token, degrees_to_cardinal, c_to_f
...
class TestMetarRoute:
    ...
    def test_iata_code_retried_with_k_prefix(self, client):
        """3-letter IATA code (SFO) should fall back to KSFO when first attempt returns nothing."""
        def side_effect(url, timeout):
            mock = MagicMock()
            mock.raise_for_status = MagicMock()
            # Return empty for SFO, real data for KSFO
            mock.text = '' if 'ids=SFO' in url else 'KSFO 251756Z 28010KT 10SM CLR 16/09 A2999'
            return mock
